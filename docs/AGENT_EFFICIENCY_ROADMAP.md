...
extractors as well, including basic Java/PHP methods and Rust functions inside `impl`; qualified
method identities, constructors, and deeper ecosystem-specific adapters remain planned. A
reproducible local A/B benchmark runner is implemented in `ai-dev benchmark`. Versioned real-agent
...
External research and current provider guidance have been reviewed in
`docs/TOKEN_EFFICIENCY_RESEARCH.md`. Its selective-retrieval, observation-lifecycle,
cache-layout, token-accounting, and bounded-refinement tasks are tracked in `TODO.md`.
