...
- `usage_status` reads at most 1,000 normalized sessions and returns aggregate policy evidence.
- `optimize_usage` reads the same bounded local sessions, limits every attribution dimension to 25
  groups, and returns explicit evidence gaps instead of guessing when quality or cost data is weak.
