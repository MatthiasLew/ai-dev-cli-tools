...
coalesced. Changes made while validation is running are queued for the next pass, so an older
result never suppresses a newer project state. Generated and local state roots such as .git,
.ai, virtual environments, caches, build output, and node_modules are ignored.
...
validation before the first file change. A value of zero keeps watching until interrupted.
