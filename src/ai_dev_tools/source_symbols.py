...
def extract_source_symbols(source: str, suffix: str) -> list[SourceSymbol] | None:
    """Return conservative top-level symbol ranges for a supported source file."""
    ...
    if normalized == ".py":
        try:
            ...
            return None
        return [
            SourceSymbol(node.name, _symbol_kind(node), node.lineno, _end_line(node))
...
def _javascript_definitions(cleaned: str, line_depths: list[int]) -> list[SourceSymbol]:
    patterns = (
        (
            re.compile(
